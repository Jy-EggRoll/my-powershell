<#
.SYNOPSIS
创建文件或文件夹的符号链接，支持批量操作和自定义路径，并可记录和检查链接状态

.DESCRIPTION
该脚本可以为指定文件或文件夹创建符号链接，支持批量选择和路径自定义，
同时记录已创建的链接（格式：符号链接绝对路径 -> 源相对路径）并提供检查功能，
确保已处理的源路径及其子路径被正确排除

不推荐在软链接路径下启动该脚本，务必保持启动脚本位置为数据源目录

由于创建软连接是高权限功能，请使用管理员身份运行脚本
#>
...
$functionScriptPath = Join-Path -Path $scriptDirectory -ChildPath "SymbolicLinkFunctions.ps1"
...
# 符号链接记录文件路径
$logFilePath = Join-Path -Path $PWD.Path -ChildPath "symlinks_windows.log"
...
# 检查模式
if ($Check) {
    Confirm-Symlinks -logFilePath $logFilePath -recordedLinks $recordedLinks
    exit 0
}

# 确定项目类型（文件/文件夹）
$itemType = if ($File) { "文件" } else { "文件夹" }

# 显示当前模式
$modeInfo = if ($File) {
    if ($NoRecurse) { "文件模式（非递归）" } else { "文件模式（递归）" }
}
else {
    if ($NoRecurse) { "文件夹模式（非递归）" } else { "文件夹模式（递归）" }
}
...
while ($true) {
    # 获取符合条件的项目
    $items = Get-ItemsToProcess -isFileMode $File -NoRecurse $NoRecurse -recordedSourceDirs $recordedSourceDirs
    ...
    # 询问用户选择，花括号包裹变量
    $userInput = Read-Host "`n请输入要创建符号链接的${itemType}编号（空格分隔多个，输入 q 退出）"
    ...
    # 路径选择菜单
    Write-Host "`n请选择符号链接创建位置：" -ForegroundColor Cyan
    ...
    foreach ($index in $validSelections) {
        ...
        $result = New-Symlink -sourcePath $sourceItem.FullName -basePath $basePath `
        ...
        if ($result) {
            Write-Host "[成功] 符号链接已创建或已存在（路径：$($sourceItem.FullName)）" -ForegroundColor Green
        }
        else {
            Write-Host "[失败] 符号链接创建未完成（路径：$($sourceItem.FullName)）" -ForegroundColor Red
        }
